...
def vector_rasterize(dir_vector, dir_raster, dir_out, flag_output): #Helper function for data_labeling
    ...
    rasterized = features.rasterize(geom_value, out_shape = raster.shape, transform = raster.transform, 
                                    all_touched = True, fill = 9, merge_alg = MergeAlg.replace, dtype = np.float32)
    if flag_output:
        with rasterio.open(
                dir_out, "w",
                driver = "GTiff",
